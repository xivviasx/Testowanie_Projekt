...
public class Calculator {
    ...
    public double convertFarenheitToCelsius (int farenheit) {
        ...
        //return Math.round(1*(farenheit-32)); // => mutacja 1
        //return 0.5555555555555*(farenheit-32); // => mutacja 2
    }
    ...
    public int factorial(int i) {
        if (i < 1) return 1;
            // if (true) return 1; // => mutacja 4
        else return i * factorial (i - 1);
        //else return i / factorial (i - 1); // => mutacja 3
    }

    public int fibbonacci (int i) {
        if (i == 0 || i == 1) return i;
        return fibbonacci (i-1) + fibbonacci (i-2);
        //return fibbonacci (i-1) - fibbonacci (i-2); // => mutacja 5
    }
